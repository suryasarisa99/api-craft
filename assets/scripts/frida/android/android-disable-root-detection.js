...
(() => {
      ...
      Interceptor.attach(fopen, {
        ...
        onLeave(retval) {
          if (retval.toInt32() !== 0) {
            ...
            if (
              ROOT_INDICATORS.paths.has(this.path) ||
              ...
              if (DEBUG_MODE) {
                console.log(
                  `Blocked possible root-detection: fopen ${this.path}`
                );
        ...
        onLeave(retval) {
          if (retval.toInt32() === 0) {
            ...
            if (
              ...
              if (DEBUG_MODE) {
                console.debug(
                  `Blocked possible root detection: access ${this.path}`
                );
              } else logFirstRootDetection();
      ...
      Interceptor.attach(stat, {
        ...
        onLeave(retval) {
          ...
          if (
            ...
            if (DEBUG_MODE) {
              console.debug(
                `Blocked possible root detection: stat ${this.path}`
              );
            } else logFirstRootDetection();
            retval.replace(ptr(-1));
          }
      ...
      Interceptor.attach(lstat, {
        ...
        onLeave(retval) {
          ...
          if (
            ...
            if (DEBUG_MODE) {
              console.debug(
                `Blocked possible root detection: lstat ${this.path}`
              );
            } else logFirstRootDetection();
            retval.replace(ptr(-1));
          }
  ...
  function bypassJavaFileCheck() {
    function isRootIndicatorFile(file) {
      ...
      return (
        ROOT_INDICATORS.paths.has(path) ||
        path.includes("magisk") ||
        filename === "su"
      );
    }
    ...
    UnixFileSystem.checkAccess.implementation = function (file, access) {
      if (isRootIndicatorFile(file)) {
        if (DEBUG_MODE) {
          console.debug(
            `Blocked possible root detection: filesystem access check for ${file.getAbsolutePath()}`
          );
        } else logFirstRootDetection();
        return false;
      }
    ...
    File.exists.implementation = function () {
      if (isRootIndicatorFile(this)) {
        if (DEBUG_MODE) {
          console.debug(
            `Blocked possible root detection: file exists check for ${this.getAbsolutePath()}`
          );
        } else logFirstRootDetection();
        return false;
      }
    ...
    File.length.implementation = function () {
      if (isRootIndicatorFile(this)) {
        if (DEBUG_MODE) {
          console.debug(
            `Blocked possible root detection: file length check for ${this.getAbsolutePath()}`
          );
        } else logFirstRootDetection();
        return 0;
      }
    ...
    FileInputStream.$init.overload("java.io.File").implementation = function (
      ...
      if (isRootIndicatorFile(file)) {
        if (DEBUG_MODE) {
          console.debug(
            `Blocked possible root detection: file stream for ${file.getAbsolutePath()}`
          );
        } else logFirstRootDetection();
        throw Java.use("java.io.FileNotFoundException").$new(
      ...
      Interceptor.attach(system_property_get, {
        ...
        onLeave(retval) {
          ...
          if (secureValue !== undefined) {
            if (DEBUG_MODE) {
              console.debug(
                `Blocked possible root detection: system_property_get ${this.key}`
              );
            } else logFirstRootDetection();
  ...
  function bypassRootPackageCheck() {
    ...
    ApplicationPackageManager.getPackageInfo.overload(
      ...
      if (ROOT_INDICATORS.packages.has(str)) {
        if (DEBUG_MODE) {
          console.debug(
            `Blocked possible root detection: package info for ${str}`
          );
        } else logFirstRootDetection();
    ...
    ApplicationPackageManager.getInstalledPackages.overload(
      ...
      const filteredPackages = packageList.filter(
        (pkg) => !ROOT_INDICATORS.packages.has(pkg.packageName?.value)
      );
      return Java.use("java.util.ArrayList").$new(
        Java.use("java.util.Arrays").asList(filteredPackages)
      );
    };
  }
  ...
  function bypassShellCommands() {
    ...
    ProcessBuilder.command.overload("java.util.List").implementation =
      function (commands) {
        const cmdArray = commands.toArray();
        if (cmdArray.length > 0) {
          const cmd = cmdArray[0].toString();
          if (
            ROOT_INDICATORS.commands.has(cmd) ||
            (cmdArray.length > 1 &&
              ROOT_INDICATORS.binaries.has(cmdArray[1].toString()))
          ) {
            if (DEBUG_MODE) {
              console.debug(
                `Blocked possible root detection: ProcessBuilder with ${cmdArray.join(
                  " "
                )}`
              );
            } else logFirstRootDetection();
    ...
    Runtime.exec.overload("[Ljava.lang.String;").implementation = function (
      ...
      if (cmdArray.length > 0) {
        ...
        if (
          ROOT_INDICATORS.commands.has(cmd) ||
          (cmdArray.length > 1 && ROOT_INDICATORS.binaries.has(cmdArray[1]))
        ) {
          if (DEBUG_MODE) {
            console.debug(
              `Blocked possible root detection: Runtime.exec for ${cmdArray.join(
                " "
              )}`
            );
          } else logFirstRootDetection();
    ...
    ProcessImpl.start.implementation = function (
      ...
      if (cmdArray.length > 0) {
        ...
        if (
          ROOT_INDICATORS.commands.has(cmd) ||
          ROOT_INDICATORS.binaries.has(arg)
        ) {
          if (DEBUG_MODE) {
            console.debug(
              `Blocked possible root detection: ProcessImpl.start for ${cmdArray.join(
                " "
              )}`
            );
          } else logFirstRootDetection();
          return ProcessImpl.start.call(
            this,
