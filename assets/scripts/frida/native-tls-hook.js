...
function patchTargetLib(targetModule, targetName) {
  ...
  const buildVerificationCallback = (realCallbackAddr) => {
    if (!verificationCallbackCache[realCallbackAddr]) {
      const realCallback =
        !realCallbackAddr || realCallbackAddr.isNull()
      ...
      const hookedCallback = new NativeCallback(
        function (ssl, out_alert) {
          ...
          for (let i = 0; i < sk_num(peerCerts); i++) {
            ...
            const certData = new Uint8Array(
              certPointer.readByteArray(certDataLength)
            );
  ...
  customVerifyAddrs.forEach((set_custom_verify_addr) => {
    ...
    Interceptor.replace(
      set_custom_verify_fn,
      new NativeCallback(
        function (ssl, mode, providedCallbackAddr) {
          set_custom_verify_fn(
            ssl,
            mode,
            buildVerificationCallback(providedCallbackAddr)
          );
        },
        "void",
        ["pointer", "int", "pointer"]
  ...
  if (customVerifyAddrs.length) {
    if (DEBUG_MODE) {
      console.log(
        `[+] Patched ${customVerifyAddrs.length} ${targetName} verification methods`
      );
    }
  ...
  const get_psk_identity_addr = targetModule.findExportByName(
  ...
  if (get_psk_identity_addr) {
    ...
    console.log(
      `Patched ${customVerifyAddrs.length} custom_verify methods, but couldn't find get_psk_identity`
    );
  }
}
