...
try {
  connectFn =
    ...
    Process.findModuleByName("libsystem_kernel.dylib")?.findExportByName(
      "connect"
    ); // iOS
} catch (e) {
...
if (!connectFn) {
  ...
  Interceptor.attach(connectFn, {
    onEnter(args) {
      ...
      if (isTCP || isUDP) {
        ...
        if (shouldBeBlocked) {
          ...
          console.log(
            `Manually intercepting connection to ${getReadableAddress(
              hostBytes,
              isIPv6
            )}:${port}`
          );
    ...
    onLeave: function (result) {
      ...
      console.debug(
        `${this.state} ${sockType} fd ${fd} to ${JSON.stringify(
          address
        )} (${result.toInt32()})`
      );
    },
  ...
  console.log(
    `== Redirecting ${
      IGNORED_NON_HTTP_PORTS.length === 0 ? "all" : "all unrecognized"
