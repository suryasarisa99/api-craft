...
if (DEBUG_MODE) {
  ...
  console.log("\n*** Starting scripts ***");
  if (globalThis.JavaBridge?.available) {
    JavaBridge.perform(() => {
  ...
  .forEach((dlopen) => {
    Interceptor.attach(dlopen.address, {
      ...
      onLeave(retval) {
        ...
        let module =
          Process.findModuleByName(this.moduleName) ??
...
const getAnonymousModule = (name, path, handle) => {
  ...
  if (!dlsymAddr) {
    console.error(
      `[!] Cannot find dlsym, cannot get anonymous module info for ${name}`
    );
    return;
  }

  const dlsym = new NativeFunction(dlsymAddr, "pointer", [
    "pointer",
    "pointer",
  ]);
